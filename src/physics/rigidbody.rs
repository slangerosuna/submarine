...
pub struct RigidBody {
    ...
    //-0.5 * air density * coefficient of Drag
...
impl RigidBody {
    ...
    //TODO add code for steering effects
    
    //TODO add code for thrust effects

    pub fn new
        (position: Vector3, rotation: Matrix4, mass: f32, 
         inertia_tensor: Matrix4, drag_coefficient: f32,
         cross_sectional_area: f32, air_density: f32,
    ...
    pub fn adv_timestep
        (&mut self, dt: f32) {
            ...
            let normalized_torque_accumulator = 
                self.torque_accumulator
                .normalized();
            
            let moment_of_inertia = 
                Matrix4::from_vector(
                    &normalized_torque_accumulator).
                multiply(&self.inertia_tensor).
                to_vector().
                dot(&normalized_torque_accumulator);
            
            //uses left hand approximation for ease of use 
            //due to relatively constant angular acceleration
            self.angular_velocity = 
                self.angular_velocity
                .add(&self.torque_accumulator
                    .multiply(dt / moment_of_inertia));
